...
/*
Modified signature created by manager with mhash=0 and wots_check_sum=0
                     ...
                     unsigned long long mlen, unsigned char *enc_label)
{
    ...
    *smlen = params->sig_bytes + mlen;
    ...
    hash_message(params, mhash, sm + params->index_bytes, pub_root, idx,
                 sm + params->sig_bytes - params->padding_len - 3*params->n,
                 mlen);
    //memset(mhash,0,mlen);   //make the hash value zero to get the component secret keys of wots
    sm += params->index_bytes + params->n;
    ...
    for (i = 0; i < params->d; i++) {
        ...
        //if(i==0){
            //printf("root = %s\n",root);
            //wots_sign_dgmtM(params, sm, root, sk_seed, pub_seed, ots_addr);
