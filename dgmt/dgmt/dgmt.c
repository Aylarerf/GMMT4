...
uint32_t sign_dgmtU(const xmss_params *params, unsigned char *sm, unsigned char *m, uint32_t id){
    ...
    if((fd = open(pathname, O_RDWR, USER_PERMISSION))==-1){
        ...
        if(read(fd, &count, sizeof(uint32_t))!=sizeof(uint32_t)){
		    printf("File: %s B_KEYS_PER_USER count read error\n",pathname);
			exit(0);
		}
        ...
        if(count < B_KEYS_PER_USER){
	        ...
	        if(write(fd, &count, sizeof(uint32_t))!=sizeof(uint32_t)){
		        printf("File: %s B_KEYS_PER_USER count write error\n",pathname);
			    exit(0);
		    }
		    close(fd);
...
void verify_dgmtU(const xmss_params *imt_params, const xmss_params *params, unsigned char *sm, unsigned char *imt_root){
	...
	vd = xmssmt_core_sign_open_dgmt(params,mout,&mlen,smd,params->sig_bytes + XMSS_MLEN,smtd_pk);
	...
	vu = xmssmt_core_sign_open(params,mout,&mlen,smu,params->sig_bytes + XMSS_MLEN,smtu_pk);
    ...
    vi = memcmp(imt_i0i1,imt_root,imt_node_len);
