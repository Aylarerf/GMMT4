...
/*Following functions are added by Sabyasachi Karati*/
/*void set_smt_tree_addr(uint32_t addr[8], uint64_t chain)
{
    addr[0] = (uint32_t) (chain & 0xFFFF); //Leaf index of SMT top layer
    chain = chain >> 16;
    addr[1] = (uint32_t) (chain & 0xFFFF); //instance of the SMT associated a particular IMT node
    chain = chain >> 16;
    addr[2] = (uint32_t) (chain & 0xFFFFFF);//Position of the IMT node in a particular layer
    chain = chain >> 24;
    addr[3] = (uint32_t) (chain & 0xFF); //height of the IMT node
}*/
