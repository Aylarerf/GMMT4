...
int create_fallback_keys(const xmss_params *params, imt_node *head, const unsigned char *inseed){
    ...
    //unsigned char	fallbacknode[2*AES_BLOCK_SIZE];
    AES_KEY         dec_key;
    ...
    if((fd = open("./dgmmt/fallback/FallBackKeys", O_WRONLY | O_CREAT | O_TRUNC, 0660))==-1){
            printf("File: ./dgmmt/fallback/FallBackKeys creation error from create_fallback_keys\n");
            exit(0);
    }else{
    	printf("From create_fallback_keys\n");
    
    	//fk = fopen("./dgmmt/fallback/FallBackKeys","w");
		...
		for(imt_i0 = imt_tree_height; imt_i0>0; imt_i0--){ 
		    ...
		    for(imt_i1 = 0; imt_i1<l; imt_i1++){
		        for(smt_j = 0; smt_j<SMT_PER_IMT_NODE; smt_j++){
		        	//printf("i0 = %u i1 = %u j = %u\n",imt_i0,imt_i1,smt_j);
		        	set_hash_addr(addr, 0);
					set_key_and_mask(addr, 0);
		            //smt_addr = (uint64_t) imt_i0 << 56;
		            //smt_addr = smt_addr | ((uint64_t) imt_i1 << 32);
		            //smt_addr = smt_addr | ((uint64_t) smt_j<< 32);
		            smt_addr[0] = 0;
		            smt_addr[1] = smt_j;
		            smt_addr[2] = imt_i1;
		            smt_addr[3] = imt_i0;
		            ...
		            /*if(write(fd, smt_pk,params->pk_bytes)!=params->pk_bytes){
                		printf("File: ./dgmmt/fallback/FallBackKeys writing error 1\n");
                		exit(0);
            		}
            		
            		if(write(fd, head[i0i1].value,imt_node_len)!=imt_node_len){
                		printf("File: ./dgmmt/fallback/FallBackKeys writing error 1\n");
                		exit(0);
            		}*/
            		
		            //make the root of SMT as the 32-byte decryption Key
		            ...
		            //fwrite(fallbackkey, 2*AES_BLOCK_SIZE, 1, fk);
